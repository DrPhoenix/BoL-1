...
function OnLoad()	
	...
	FCConfig:addParam("autoParry", "Auto Parry", SCRIPT_PARAM_ONOFF, true)
	--FCConfig:addParam("AutoE", "Auto double AA, when AA Enemy", SCRIPT_PARAM_ONOFF, false)
	FCConfig:addParam("autoks", "Auto kill with Lunge", SCRIPT_PARAM_ONOFF, false)
...
function OnTick()
	...
	-- AA E
	if FCConfig.AutoE and (os.clock() - lastBasicAttack) > swingDelay and (os.clock() - lastBasicAttack) < (swingDelay * 2) and (lastBasicAttackTarget.type == "AIHeroClient" or lastBasicAttackTarget.type == "Turret_") and myHero:CanUseSpell(_E) == READY and GetDistance(lastBasicAttackTarget) <= eBuffer  then
...
function OnProcessSpell(unit, spell)
        ...
        if FCConfig.autoParry then
                if unit ~= nil and unit.type == "AIHeroClient" and GetDistance(spell.endPos) <= 50 and unit.team ~= myHero.team and not unit.isMe then
