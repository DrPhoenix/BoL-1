...
do
	...
	function getDmg(spellname,SDLtarget,SDLplayer,stagedmg,spelllvl)
		...
		local lvl = SDLplayer.level
		local ap = SDLplayer.ap
		local ad = SDLplayer.totalDamage
		...
		local Qlvl = SDLplayer:GetSpellData(_Q).level
		local Wlvl = SDLplayer:GetSpellData(_W).level
		local Elvl = SDLplayer:GetSpellData(_E).level
		local Rlvl = SDLplayer:GetSpellData(_R).level
		...
		elseif (spellname == "DFG") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,.15*tmhp)
		elseif (spellname == "HXG") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,150+.4*ap)
		elseif (spellname == "BWC") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,150)
		elseif (spellname == "KITAES") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,.025*tmhp)
		elseif (spellname == "MALADY") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,15+.1*ap)
		elseif (spellname == "WITSEND") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,42)
		elseif (spellname == "SHEEN") then
			TrueDmg = SDLplayer:CalcDamage(SDLtarget,ad-bad) --(bonus)
		elseif (spellname == "TRINITY") then
			TrueDmg = SDLplayer:CalcDamage(SDLtarget,(ad-bad)*1.5) --(bonus)
		elseif (spellname == "LICHBANE") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,50+.75*ap) --(bonus)
		elseif (spellname == "LIANDRYS") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,.05*thp) --over 3 sec, If their movement is impaired, they take double damage from this effect. Half duration for multi-target or periodic effects
		elseif (spellname == "BLACKFIRE") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,.035*tmhp) --over 2 sec
		elseif (spellname == "STATIKK") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,100)
		elseif (spellname == "ICEBORN") then
			TrueDmg = SDLplayer:CalcDamage(SDLtarget,(ad-bad)*1.25) --(bonus)
		elseif (spellname == "TIAMAT") then
			TrueDmg = SDLplayer:CalcDamage(SDLtarget,.6*ad) --decaying down to 33.33% near the edge (20% of ad)
		elseif (spellname == "HYDRA") then
			TrueDmg = SDLplayer:CalcDamage(SDLtarget,.6*ad) --decaying down to 33.33% near the edge (20% of ad)
		elseif (spellname == "RUINEDKING") then
			TrueDmg = math.max(SDLplayer:CalcMagicDamage(SDLtarget,.04*thp)*(stagedmg1+stagedmg3),SDLplayer:CalcDamage(SDLtarget,150+.5*ad)*stagedmg2) --stage1:Passive. stage2:Active. stage3: stage1
		elseif (spellname == "MURAMANA") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,.06*mana)
		elseif (spellname == "HURRICANE") then
			TrueDmg = SDLplayer:CalcDamage(SDLtarget,10+.5*ad) --apply on-hit effects
		elseif (spellname == "SPIRITLIZARD") then
			TrueDmg = 12+3*lvl --over 3 sec
		elseif (spellname == "SUNFIRE") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,40) --x sec
		elseif (spellname == "LIGHTBRINGER") then
			TrueDmg = SDLplayer:CalcMagicDamage(SDLtarget,100) -- 20% chance
