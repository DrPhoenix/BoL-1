	...
	if not Menu.drawMaster then
		...
		if not myHero.dead then
			if QREADY and Menu.drawQ then
				if myHero.range > 400 then DrawCircle(myHero.x, myHero.y, myHero.z, qRange, 0x990099)
					else DrawCircle(myHero.x, myHero.y, myHero.z, qRange2, 0x00FF00)
				end
			end
			if EREADY and Menu.drawE then
				DrawCircle(myHero.x, myHero.y, myHero.z, eRange, 0x009900)
			end
		end
	end
end
...
function mainMenu()
	...
	Menu:addParam("drawE", "Draw - Vault", SCRIPT_PARAM_ONOFF, true)
end
...
--UPDATEURL=
--HASH=60E02B09E95110DB8EB20635F5186055
